...
Проект развивает предыдущие лабораторные:
...
3. Откройте:
   - сайт: `http://localhost/`
   - админку: `http://localhost/admin/`

## Что показать на сдаче
1. `compose.yaml` — разбиение на сервисы.
2. `Dockerfile` и `docker/nginx/default.conf` — контейнеризация и reverse proxy.
3. `.github/workflows/ci-cd.yml` — CI/CD.
4. `main/tasks.py` — вынос Telegram в отдельный worker-сервис.
5. `/` в двух окнах — WebSocket-комментарии без перезагрузки.
6. `/contacts/` — регистрация пользователя.
7. `/admin/` — Users, Directions, Registrations, Comments.

## Локальная CI-проверка без GitHub
```powershell
python -m venv .venv
